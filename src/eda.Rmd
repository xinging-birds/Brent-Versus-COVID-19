...
# install.packages(c("tidyquant", "car", "cowplot"))
library(tidyverse)
library(tidyquant)
library(car)
library(cowplot)
...
```{r}
logistic_model = function(nls_model, xvar, df) {
  ...
  # Get x value range
  x <- c(min(df[xvar]):(max(df[xvar])+50))
...
Function inspired by the logistic growth model example by [Brian Cheng](https://bscheng.com/2014/05/07/modeling-logistic-growth-data-in-r/). The actual creation of the logit model by `nls()` is omitted from the function because creating complicated formulas using strings is hard :(.
...
```{r}
rss <- sum(residuals(hubei_nls)^2)
tss <- sum((Hubei$confirmed - mean(Hubei$confirmed))^2)
abs(tss-rss)/tss # Calculation of r^2
```

That's a beautiful r-squared value. 

Interestingly, there is little effect on the Brent Crude price, since in the range where the cases grew the most, there was almost no net change in Brent Crude up to day 30 (since the prices rebounded) and then began to dip as cases in China began to stabilize. So what *did* affect the oil price?
...
start <- coef(lm(logit(confirmed/100) ~ datediff, data = Italy)) 

# Important to note that the carrying capacity asymptote, 100000, was an assumption
italy_nls <- nls(confirmed ~ phi1 / (1 + exp(-(phi2 + phi3 * datediff))),
              start = list(phi1 = 10000000, phi2 = start[[1]], phi3=start[[2]]), 
              data = Italy)

italy_logit <- logistic_model(italy_nls, "datediff", America)
...
italy_graph <-
  Italy %>%
    ggplot(aes(datediff, confirmed))+
      geom_point(color='blue') +
      labs(x='Days Since 1-22-2020', y='Confirmed Cases') +
      geom_line(data=italy_logit, aes(x, y)) +
...
rss <- sum(residuals(italy_nls)^2)
tss <- sum((Italy$confirmed - mean(Italy$confirmed))^2)
abs(tss-rss)/tss # Calculation of r^2
```
...
Italy %>%
  ggplot(aes(datediff, confirmed)) +
    ...
    scale_y_log10() +
    geom_line(data=italy_logit, aes(x, y), color="red") +
    theme_minimal()
```
...
start <- coef(lm(logit(confirmed/100) ~ datediff, data = America)) 

us_nls <- nls(confirmed ~ phi1 / (1 + exp(-(phi2 + phi3 * datediff))),
              start = list(phi1 = 150000000, phi2 = start[[1]], phi3=start[[2]]), 
              data = America)

us_logit <- logistic_model(us_nls, "datediff", America)
...
us_graph <-
  America %>%
    ggplot(aes(datediff, confirmed))+
      geom_point(color='blue') +
      labs(x='Days Since 1-22-2020', y='Confirmed Cases') +
      geom_line(data=us_logit, aes(x, y)) +
      geom_vline(aes(xintercept=50), linetype=3) +
      geom_vline(aes(xintercept=72), color = "green") +
      geom_vline(aes(xintercept=95), linetype=3) +
      xlim(40, 120) +
      theme_minimal()

plot_grid(us_graph, brent_graph + geom_vline(aes(xintercept=72), color="green") + xlim(40, 80))

rss <- sum(residuals(us_nls)^2)
tss <- sum((America$confirmed - mean(America$confirmed))^2)
abs(tss-rss)/tss # Calculation of r^2
```
